    ...
    def evaluate(self, test_loaders):
        ...
        if self.params.trick['ncm_trick'] or self.params.agent in ['ICARL', 'SCR', 'SCP']:
            ...
            for cls, exemplar in cls_exemplar.items():
                ...
                for ex in exemplar:
                    if self.params.agent in ['DSD','PCR']:
                        feature, _ = self.model.features(ex.unsqueeze(0))
                ...
                if len(features) == 0:
                    if self.params.agent in ['DSD','PCR']:
                        feature_0, _ = self.model.features(x.unsqueeze(0))
                        mu_y = maybe_cuda(torch.normal(0, 1, size=tuple(feature_0.detach().size())), self.cuda)
                    else:
        ...
        with torch.no_grad():
            ...
            for task, test_loader in enumerate(test_loaders):
                ...
                for i, (batch_x, batch_y) in enumerate(test_loader):
                    ...
                    if self.params.trick['ncm_trick'] or self.params.agent in ['ICARL', 'SCR', 'SCP']:
                        if self.params.agent in ['DSD','PCR']:
                            feature, _ = self.model.features(batch_x)  # (batch_size, feature_size)
                        else:    
                    ...
                    elif self.params.agent in ['DSD','PCR']:
                        logits, _, _ = self.model.pcrForward(batch_x)
